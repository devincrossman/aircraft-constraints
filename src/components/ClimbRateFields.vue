<template>
  <fieldset>
    ...
    <label><Equation data="$C_{D_{induced}}$" title="Lift induced drag coefficient"/>
      <input type="range" min="0" max="0.05" step=".005"
        v-model.number="constraint.liftInducedDragCoeff "/>
      <input v-model.number="constraint.liftInducedDragCoeff"/>
    </label>
  </fieldset>
</template>
...
import { ClimbRateConstraint } from '@/interfaces/constraints';
...
export default class ClimbRateFields extends Vue {
  @Prop() constraint!: ClimbRateConstraint;
}
</script>
